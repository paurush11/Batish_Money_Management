...
export const RegisterForm: React.FC<TRegisterFormProps> = ({}) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    console.log("clicked");
    const requiredValues: IUser = {
      firstName: values.firstName,
      lastName: values.lastName,
      userName: values.username,
      email: values.email,
      phoneNumber:
        values.phoneNumberFirst +
        "-" +
        values.phoneNumberSecond +
        "-" +
        values.phoneNumberLast,
      password: values.password,
      role: "USER",
    };
    console.log(requiredValues);
    try {
      const res = await context?.register(requiredValues as IUser);
      if (res && res.message) {
        toast({
      ...
      toast({
        title: "Error in Registering User",
        variant: "destructive",
        description: `New User Named ${e.message} has Not been added Successfully`,
      });
    }
  };
  const FirstName = (
